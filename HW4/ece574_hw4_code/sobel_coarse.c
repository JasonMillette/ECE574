...
static void *generic_convolve(void *argument) {
	...
	for (int x=1; x<old->x-1; x++) {
		for (int y=1; y<old->y-1; y++) {
...
int main(int argc, char **argv) {
	...
	pthread_t threads[NUM_THREADS]; //inits number of threads from define
	...
	value[0] = PAPI_get_real_usec(); //first time stamp
	pthread_create(&threads[0], &attr, generic_convolve, (void *)&sobel_data[0]);
	pthread_create(&threads[1], &attr, generic_convolve, (void *)&sobel_data[1]);
