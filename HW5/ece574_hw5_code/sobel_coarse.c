...
int main(int argc, char **argv) {
	...
	/* convolution */
	value[0] = PAPI_get_real_usec(); //first time stamp
#pragma omp parallel shared(sobel_y, sobel_x)  //inits omp parallel section, with the sobels as shared
	{
	#pragma omp sections nowait //inits sections
		{
		#pragma omp section //section 1 does sobel x
			generic_convolve(&image,&sobel_x, sobel_x_filter);
		#pragma omp section //section 2 does sobel y
