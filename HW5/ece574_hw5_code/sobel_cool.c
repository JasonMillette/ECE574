...
static void generic_convolve(struct image_t *input_image,
	...
	/******************/
	int sum;
#pragma omp parallel shared(input_image, output_image) private(sum)	//inits parralel section using OMP
	{
	#pragma omp for simd schedule(static, 1) nowait	//creates threads using simd
...
static int combine(struct image_t *sobel_x, struct image_t *sobel_y,
	...
	/******************/
	int sum;
#pragma omp parallel shared(sobel_y, sobel_x) private(sum)	//inits OMP section for parallel section
	{
	#pragma omp for simd schedule(static, 1) nowait		//creates threads using simd
